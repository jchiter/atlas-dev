...
( function( api ) {
	var cssTemplate = wp.template( 'homelist-color-scheme' ),
		colorSchemeKeys = [
			'background_color',
		...
		ready: function() {
			if ( 'color_scheme' === this.id ) {
				this.setting.bind( 'change', function( value ) {
					...
					color = colors[1];
					api( 'object_background_color' ).set( color );
					api.control( 'object_background_color' ).container.find( '.color-picker-hex' )
						.data( 'data-default-color', color )
						.wpColorPicker( 'defaultColor', color );
                    /*
					// Update Link Color.
