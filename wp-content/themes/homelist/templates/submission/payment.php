...
<article id="page-<?php the_ID(); ?>" <?php post_class( 'submission-payment' ); ?>>
    ...
    <?php if ( 'pay_for_featured' == $payment_type ) : ?>
        ...
        <div class="payment-info">
			<?php echo sprintf( esc_html__( 'You are going to pay <strong>%s</strong> for featuring property <strong>"%s"</strong>.', 'homelist' ), Realia_Price::format_price( $price ), $property->post_title ); ?>
        </div><!-- /.payment-info -->
    <?php elseif ( 'pay_for_sticky' == $payment_type ) : ?>
        ...
        <div class="payment-info">
            <?php echo sprintf( esc_html__( 'You are going to pay <strong>%s</strong> for TOP property <strong>"%s"</strong>.', 'homelist' ), Realia_Price::format_price( $price ), $property->post_title ); ?>
        </div><!-- /.payment-info -->
    <?php elseif ( 'pay_per_post' == $payment_type ) : ?>
        ...
        <div class="payment-info">
            <?php echo sprintf( esc_html__( 'You are going to pay <strong>%s</strong> for publishing property <strong>"%s"</strong>.', 'homelist' ), Realia_Price::format_price( $price ), $property->post_title ); ?>
        </div><!-- /.payment-info -->
    <?php elseif ( 'package' == $payment_type ) : ?>
        <?php if ( ! empty( $object_id ) ) : ?>
            ...
            <div class="payment-info">
                <?php echo sprintf( esc_html__( 'You are going to pay <strong>%s</strong> for package <strong>"%s"</strong>.', 'homelist' ), Realia_Price::format_price( $price ), $title ); ?>
            </div><!-- /.payment-info -->
        <?php else : ?>
            <div class="payment-info">
                <?php echo esc_html__( 'Missing package.', 'homelist' ); ?>
            </div><!-- /.payment-info -->
        <?php endif; ?>
    <?php else : ?>
        <div class="payment-info">
            <?php echo esc_html__( 'You are missing payment type.', 'homelist' ); ?>
        </div><!-- /.payment-info -->
    <?php endif; ?>
        ...
        <form class="payment-form" method="post" action="?">
            ...
            <?php if ( is_array( $payment_gateways ) && count( $payment_gateways ) > 0 ) : ?>
                <?php foreach ( $payment_gateways as $gateway ) : ?>
                    <div class="gateway">
                        ...
                        <?php if ( ! empty( $gateway['content'] ) ) : ?>
                            <div class="gateway-content"><?php echo wp_kses_post( $gateway['content'] ); ?></div><!-- /.gateway-content -->
                        <?php endif; ?>
                    </div><!-- /.payment -->
                <?php endforeach; ?>
                ...
                <?php if ( ! empty( $payment_type ) ) : ?>
                    <button type="submit" name="process-payment" class="submission-payment-process hidden">
                        <?php echo esc_html__( 'Proceed payment', 'homelist' ); ?>
                    </button>

                    <div class="alert alert-info hidden" id="non-proceed-info">
                        <?php echo esc_html__( 'Your submission will be manually reviewed when transfer will be completed.', 'homelist' ); ?>
                    </div>
                <?php endif; ?>
            <?php else : ?>
                <div class="alert alert-warning">
                    <?php echo esc_html__( 'No payment gateways found.', 'homelist' ); ?>
                </div><!-- /.alert -->
            <?php endif; ?>
            ...
            <?php if ( ! empty( $submission_page_id ) ) : ?>
                <p class="submission-payment-back">
                    <?php echo esc_html__( 'Back to:', 'homelist' ); ?> <a href="<?php echo get_permalink( $submission_page_id ); ?>">
